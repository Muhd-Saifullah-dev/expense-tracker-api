...
const { check_budget_limit } = require("./check-budget-service");
...
const get_single_transaction = async (req, res, next) => {
  try {
    ...
    if (!transaction) {
      return res
        .status(404)
        .json(responses.not_found_error("transaction not found"));
    }

    return res
      .status(200)
      .json(responses.ok_response(transaction, "transaction get successfully"));
  } catch (error) {
    next(error);
  }
};
